...
import "../../css/account/address-list.css";
...
const Addresses = () => {
  ...
  useEffect(() => {
    if (localStorage.getItem("address_created") === "true") {
      console.log(localStorage.getItem("address_created"));
      handleAddNewSet(false);
    }
  }, []);
  const handleDelete = (id, data) => {
    const new1 = data.filter((item) => item.id !== id);
    console.log(new1);
    dispatch(deleteAddresses(id, new1));
  };
  const handleEditSet = (id, value) => {
    if (value === true) {
      setDefault1(true);
    } else {
      setDefault1(false);
  ...
  const handleDefaultInputValues = (id) => {
    const new1 = address.filter((item) => item.id === id);
    const new2 = new1[0];
    setFirst_name(new2.first_name);
    setLast_name(new2.last_name);
    setCompany(new2.company);
    setAddress1(new2.address1);
    setAddress2(new2.address2);
    setCity(new2.city);
    setCountry(new2.country);
    setPostal(new2.postal);
    setPhone(new2.phone);
    setDefault1(new2.default);
  };
  const handleSubmit = (id, index) => {
    ...
    dispatch(updateAddresses(data, index));
    if (localStorage.getItem("address_updated") === "true") {
      console.log(localStorage.getItem("address_updated"));
      handleDeleteClear();
    }
  };
  const handleSubmitAdd = () => {
    ...
    if (
      ...
      dispatch(addNewAddress(data, address));
      if (localStorage.getItem("address_created") === "true") {
        console.log(localStorage.getItem("address_created"));
        handleAddNewSet(false);
      }
  ...
  return (
    <React.Fragment>
      ...
      <div className="container">
        ...
        <div className="row">
          <Link to="/account" className="return-account mr-auto">
            ...
            <div className="form-inputs-container">
              ...
              >
                <div className="form-group ">
                  <label className="form-input-label" htmlFor="first_name">
                ...
                <div className="form-group ">
                  <div className="row">
                    ...
                    <span className="radio-text">Set as default address</span>
                  </div>
                </div>
              </form>
            </div>
                ...
                item.default === true ? (
                  <React.Fragment>
                    <div className="account-adresses">
                      ...
                      <span className="customer-add">{item.company}</span>
                      <span className="customer-add">{item.address1}</span>
                      <span className="customer-add">{item.address2}</span>
                    ...
                    <div className="row">
                      <Link
                        className="add-save-remove"
                        onClick={() => {
                          handleEditSet(index, item.default);
                          handleDefaultInputValues(item.id);
                        }}
                      >
                        Edit
                      </Link>
                      <Link
                        className="add-save-remove"
                        onClick={() => handleDelete(item.id, address)}
                      >
                        Remove
                      </Link>
                    </div>
